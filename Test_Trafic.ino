...
int gpio1_pin = LED_BUILTIN;  // D4 of nodemcu
int gpio2_pin = LED_BUILTIN; // D7 of nodemcu
int gpio3_pin =LED_BUILTIN; //  D5 of nodemcu
...
void handleLogin(){
  ...
  content += "<p  align ='center' style='font-size:160%'><input type='submit' name='SUBMIT' onclick='return encodeMyHtml()' value='Submit'></form>" + msg + "</p><br> </body>";
  content += "<?php session_start(); ?>";
  //.content +="<body><?php $_SESSION['csrf_token'] = uniqid(); ?></body>";
  content +="<body><form><input type='hidden' id='token' name='csrf_token' value='<?php echo uniqid();?>' />";
  //content += "</form><script>var token = document.getElementById('token').value;";
